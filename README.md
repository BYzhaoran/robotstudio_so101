...
Run `stm2sim.py` to launch the MuJoCo viewer with an external PID loop.

- The right-side MuJoCo actuator sliders set the joint targets in manual mode.
- Press `M` to toggle between automatic target switching and manual slider control.
- Press `N` / `P` to change the selected joint.
- Press `G` to cycle `Kp`, `Ki`, and `Kd`.
- Press `+` / `-` to adjust the selected gain, `R` to reset the integrators, and `[` / `]` to change the adjustment step.
